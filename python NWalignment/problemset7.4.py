...
seq_left=dna[1].rstrip()             #change the number of sequence here
seq_top=dna[2].rstrip()
...
print len(aln[0]),len(aln[1])


def percent_id(aln_seq1,aln_seq2):
    i_score=0
    t=len(aln_seq1)
    for i in range(len(aln_seq1)):      #len(aln_seq1)=len(aln_seq2)
        if aln_seq1[i]==aln_seq2[i]:
            i_score+=1
    print (float(i_score)/float(t))*100



print percent_id(aln[0],aln[1])
