...
#  Description: This program builds a binary search tree class,and applys several embedded functions such as insert, isSimilar(check for similar tree)
#getHeight() returns the height of the tree, print the tree at a certain level, and also counts for the total number of nodes
...
class Tree (object):
  ...
  def printLevel(self,level):
    if self.root == None:
      ...
      self.printLevelHelper(self.root,level,1)
    
  def printLevelHelper(self,r,desired,current):
      if r == None:
        return
      elif (desired == current):
        print (r.data,end=' ')
      else:
        self.printLevelHelper(r.lChild,desired,current+1)
        self.printLevelHelper(r.rChild,desired,current+1)
...
def main():
  ...
  print('\nTesting printing level 2 in tree3:')
  tree3.printLevel(2)

  # Get the height of the two trees that are different
  print('\n\nThe height of tree1 is:',tree1.getHeight())
  print('The height of tree3 is:',tree3.getHeight())
